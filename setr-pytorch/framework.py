...
class MyFrame():
    def __init__(self, net, loss, lr=2e-4, evalmode = False):
        self.net = net().cuda()##将网络放在cuda上进行计算
        # self.net = torch.nn.DataParallel(self.net,device_ids=range(torch.cuda.device_count()))##当有多个GPU时，可以使用多个GPU进行训练
        ##优化函数
    ...
    def test_batch(self):
        ...
        mask = self.net.forward(self.img).cpu().data.numpy().squeeze(1)##删除维度为1的维度
        mask[mask>0.5] = 1
        mask[mask<=0.5] = 0
