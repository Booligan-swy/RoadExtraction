'''
描述：transformer模块修改，借助多个transformer构成四层的编码器结构，并将每一层进行输出。目的是为了在解码器部分融合特征。（此时的
transformer内部结构未改变，还是利用的mlp，并不是卷积。）原始版本请参照transformer_model.py文件。
作者：wyshen
时间：2021.11.2
版本：v0.1
备注：
...
class TransSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # print("self attention output: ", context_layer.size())
...
class TransSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        # print("TransSelfOutput config hidden_size : ",self.config.hidden_size)
        # print("TransSelfOutput config layer_norm_eps : ",self.config.layer_norm_eps)
...
class TransModel2d(nn.Module):
    ...
    def forward(
        ...
        # print("dense out: ",dense_out.size())
        ...
        # print("transformer block output: ", encoder_layers[-1].size())
        # print("transformer block output: ", encoder_layers[-2].size())
